...
#define N_mode_ 64 // Ad hoc: we change this to 64 for swell, and have 65*65 grid. 
double F_kxky_[(N_mode_+1)*(N_mode_+1)], omega[(N_mode_+1)*(N_mode_+1)], phase[(N_mode_+1)*(N_mode_+1)];
double kx_[N_mode_+1], ky_[N_mode_+1];
double dkx_, dky_;
...
/* If using MPI. **/
  ...
  if (rank == root) {
    ...
    // Same number of modes in ky
    float * b2 = (float*) malloc (sizeof(float)*(length1D));
    sprintf (filename, "ky");
...
/**
   If not using MPI. */ 
#else
  int length2D = (N_mode_+1)*(N_mode_+1);
  float * a = (float*) malloc (sizeof(float)*length2D);
  char filename[100];
  sprintf (filename, "F_kxky");
  FILE * fp = fopen (filename, "rb");
