...
#define g_ 9.8
double h_ = 200; // depth of the water
...
#include "./spectrum_swell.h" // Used for new input method (the spectrum info)
...
int main(int argc, char * argv[])
{
  ...
  nl = NLAYER;
  G = g_;
  h_ = L0/5.; // water depth scales with domain now instead of kp because kp not defined anymore 
#if dimension == 2
  gpe_base = -0.5*sq(h_)*sq(L0)*g_;
#else
  gpe_base = -0.5*sq(h_)*L0*g_;
#endif
  CFL_H = 1; // Smaller time step (this is not used because of implicit stepping
