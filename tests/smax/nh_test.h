			...
			double * H, double * d)
{
  ...
  for (int l = 0, m = nl - 1; l < nl; l++, m--) {
    ...
    foreach_dimension() {
      #if SMAX
      if ((dz.x - h[0,0,m] + h[-1,0,m])/Delta > max_slope) {
	fprintf (stderr, "Gradient-limiter triggered within box_matrix at t=%g \
                 when (dz.x - h[0,0,m] + h[-1,0,m])/DELTA = %g in iterative layer l=%d, m=%d! \n", \
		 t, (dz.x - h[0,0,m] + h[-1,0,m])/Delta, point.l, m);
      }	
     if ((dzp.x - h[1,0,m] + h[0,0,m])/Delta > max_slope) {
	fprintf (stderr, "Gradient-limiter triggered within box_matrix at t=%g \
                 when (dzp.x - h[1,0,m] + h[0,0,m])/DELTA = %g in iterative layer l=%d, m=%d! \n", \
		 t, (dzp.x - h[1,0,m] + h[0,0,m])/Delta, point.l, m);
     } 
     #endif
    ...
    if (l > 0) {
      ...
      foreach_dimension() {
#if SMAX
	if (dz.x/Delta > max_slope) {
	fprintf (stderr, "Gradient-limiter triggered within box_matrix at t=%g \
                 when dz.x/Delta = %g in iterative layer l=%d! \n", \
	         t, dz.x/Delta, point.l);
      }	
	if (dzp.x/Delta > max_slope) {
	fprintf (stderr, "Gradient-limiter triggered within box_matrix at t=%g \
                 when dzp.x/DELTA = %g in iterative layer l=%d! \n", \
	         t, dzp.x/Delta, point.l);
      } 
#endif
...
static void relax_nh (scalar * phil, scalar * rhsl, int lev, void * data)
{
  ...
  foreach_level_or_leaf (lev) {
	...
	foreach_layer() {												
	  if (h[1] + h[0] > dry) {						
	    if (dz/Delta > max_slope) 
	      fprintf (stderr, "Gradient-limiter triggered within hpg (eta update) at t=%g \
                       when dz.x/Delta = %g in layer l=%d! \n", \
		       t, dz/Delta, point.l);
	    if (point.l < nl - 1) {						
	      if ((dz + h[1] - h[0])/Delta > max_slope) 
		fprintf (stderr, "Gradient-limiter triggered within hpg (eta update) at t=%g \
                         when (dz + h[1] - h[0])/Delta = %g in layer l=%d! \n", \
			 t, (dz + h[1] - h[0])/Delta, point.l);
	    }							
	  }		
	}
      }
  ...
  foreach (reduction(max:maxres)) {
    ...
    */
    ...
    foreach_layer() {
      ...
      foreach_dimension() {
#if SMAX
	if ((dz.x + hf.x[1] - hf.x[])/(Delta*cm[]) > max_slope) {
	fprintf (stderr, "Gradient-limiter triggered within residual_nh at t=%g \
                 when (dz.x + hf.x[1] - hf.x[])/(Delta*cm[]) = %g in layer l=%d! \n", \
	         t, (dz.x + hf.x[1] - hf.x[])/(Delta*cm[]), point.l);
      }	
	if (dz.x/(Delta*cm[]) > max_slope) {
	fprintf (stderr, "Gradient-limiter triggered within residual_nh at t=%g \
                 when dz.x/(Delta*cm[]) = %g in layer l=%d! \n", \
	         t, dz.x/(Delta*cm[]), point.l);
      } 
#endif
...
~~~
...
#if SMAX
	if (dz.x/(Delta*cm[]) > max_slope) {
	fprintf (stderr, "Gradient-limiter triggered in pressure event when computing rhs at t=%g \
                 when dz.x/(Delta*cm[]) = %g in layer l=%d! \n", \
	         t, dz.x/(Delta*cm[]), point.l);
      }	
#endif
  ...
  foreach_face() {
	...
	foreach_layer() {												
	  if (h[1] + h[0] > dry) {						
	    if (dz/Delta > max_slope) 
	      fprintf (stderr, "Gradient-limiter triggered within hpg (acceleration update) at t=%g \
                       when dz/Delta = %g in layer l=%d! \n", \
		       t, dz/Delta, point.l);
	    if (point.l < nl - 1) {						
	      if ((dz + h[1] - h[0])/Delta > max_slope) 
		fprintf (stderr, "Gradient-limiter triggered within hpg (acceleration update) at t=%g \
