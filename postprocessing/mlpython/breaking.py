...
def get_bins(kp):
    if kp == 2*np.pi/10:
        bins = np.array([0.01]); bins = np.concatenate((bins,np.arange(0.25,5,0.25)))   
    elif kp == 2*np.pi/40:
        bins = np.array([0.01]); bins = np.concatenate((bins,np.arange(0.5,10,0.25)))   
    elif kp == 2*np.pi/100:
        bins = np.array([0.01]); bins = np.concatenate((bins,np.arange(0.5,15,0.25)))   
    else:
...
def simple_mapping(eta, ux, uy, delta, bins=[], method=0, threshold=0, EXTRA_FILTER=True):
    """ Arguments: 
    ...
    if method == 0:
        ...
        eta_gradx = np.grad(eta, axis=0) / delta
